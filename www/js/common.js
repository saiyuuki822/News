...
function show_bookmark(event) {
  offset = 0;
  limit = 10;
  exec_ajax(api_root+"api/node_list/all/data","post",{"uid":user.uid, "type":"bookmark", "offset": offset, "limit": limit}, node_callback,user,set_action);
  $('#node-area').html('');
...
function theme_settings() {
  swal("Cancelled", "現在テーマは1つしか無い為、\nテーマ設定を解除出来ません。", "error");
}

function theme_callback(data) {
  console.log(data);
  $('#node-area').hide();
  $('#theme-area').html('');
  $.each(data.list, function(index, value){
    $('#theme-template').addClass('theme-'+value.theme_id);
    $('#theme-template .post').attr('id','theme-'+value.theme_id);
    $('#theme-template .theme_title').html(value.name);
    $('#theme-template .post-image').attr('src', value.image);
    var element = $('#theme-template').prop('outerHTML');
    $('#theme-area').append(element);
    
  });
  $('#theme-area #theme-template').show();
}

function show_theme(event) {
//  var myNavigator = document.getElementById('navigator');
//  myNavigator.pushPage('theme.html');
  var menu = document.getElementById('menu');
  menu.close();
  exec_ajax(api_root+"api/theme","post", {}, theme_callback);
  return false;
}

function show_site_settings(event) {
  window.open('http://saiyuuki-syokudo.sakura.ne.jp/settings/login?token='+user.token ,'_blank'); 
  return false;
}
...
function login_callback_after(data) {
    user = data;
    if($('[name="auto_login"]').prop('checked')) {
      $(document).trigger('deviceready', ['relation', user.uid]);
      $(document).trigger('deviceready', ['push', user.uid, "ログインしました。"]);
/*
      ...
      var ncmb = new NCMB("1f850cb9b203f27b63e5088466fc8cc4007a332dc9c61b2109c60f3cd4199c72","587dd7b6d228833d25588f39f4d8592e3110f0d79c63864f9c23271782bc3a5f");
                            ncmb.Installation
                              .equalTo('objectId', id)
                              .fetch()
                              .then((installation) => {
...
*/
    ...
    //alert(user.uid);
    exec_ajax_no_lock(api_root+"api/activity","get",{"uid":user.uid}, activity_callback);
...
function put_tags_menu(data) {
  if($('#menu_wrapper .tags_menu').length == 0) {
    for (var i=0; i<data.tags.length;i++) {
      //なぜか最初のタグにtags_menuクラスが付かないので後から付与する為に保持
      if(i == 0) {
        var temp_tag_id = data.tags[i].tag_id;
      }
      $('#menu_template_wrapper #menu_template').html(data.tags[i].name);
      $('#menu_wrapper #theme_menu').before($('#menu_template_wrapper #menu_template').prop('outerHTML'));
      $('#menu_template_wrapper #menu_template').addClass('tags_menu');
      $('#menu_wrapper .tags_menu:last').attr('data-tag_id', data.tags[i].tag_id);
//      $('#menu_wrapper .tags_menu:last').attr('id', '');
    }
    $('#menu_wrapper #menu_template:first').addClass('tags_menu');
    $('#menu_wrapper #menu_template:first').attr('data-tag_id', temp_tag_id);
  }
...
function login() {
 ...
 var password = $('#password').val();
   exec_ajax(api_root+"api/login","post", {"name": name,"password": password, "auto_login": $('[name="auto_login"]').prop('checked')}, login_callback);
}

function auto_login(uid) {
   exec_ajax(api_root+"api/login","post", {"uid": uid, "auto_login": false}, login_callback);
}
...
function edit_node_callback(data) { 
  ...
  $('#preview').attr('src', data[0].image);
  $('#preview').show();
  console.log(data[0].image_caption);
  var last_index = 1;
  var num = 0;
  $.each(data[0].image_caption, function(index, value) {
    if(value.field_image.length) {
      $('#preview'+(index+1)).attr('src', value.field_image_url);
      $('#preview'+(index+1)).show();
      $('#fid'+(index+1)).attr('src', value.fid);
      $('#image_delete'+(index+1)).show();
      //$('[name="field_image'+(index+1)+'"').val(value.field_image);
      $('[name="field_image'+(index+1)+'"]').hide();
      $('[name="fid'+(index+1)+'"]').val('');
      $('[name="caption'+(index+1)+'"]').val(value.caption);
      num++;
    }
    $('#image_caption_wrapper'+(index+1)).show();
    last_index = (index+1) + 1;
  });
  if(num < 4) {
      $('#move_next_caption_image_page1').hide();
  } else {
      $('#move_next_caption_image_page1').show();
  }
  $('#image_caption_wrapper'+last_index).show();
}
...
function upload_callback(data) {
  ...
  if(data.post_type !== "regist") { 
    alert("編集が完了しました。");
    $('#addpost_btn').removeAttr('disabled');
    var nid = data.nid;
    $('#node-area').html('');
...
function image_delete(event) {
  $(this).parent().find('img').hide();
  $(this).parents('.media').find('.field_image_form').show();
  $(this).hide();
  return false;
}
...
function touchmove(event) {
  ...
  if($('#area-end').length) {
    if(original.changedTouches) {
      ...
      scroll_end_x = event.pageX;
      scroll_end_y = event.pageY;
    }
...
function good_callback(data) {
  ...
  if(data.type == 'good') {
    ...
    if(user.uid != data.author_uid) {
      $(document).trigger('deviceready', ['push', data.author_uid, data.message]);
    }
  } else {
...
function follow_callback(data) {
  ...
  if(data.type == 'follow') {
    ...
    if(user.uid != data.follow_uid) {
      $(document).trigger('deviceready', ['push', data.follow_uid, data.message]);
    }
  } else {
...
function show_post_view(event) {
  $('[name=post_title]').val('');
  $('[name=post_body]').val('');
  $('[name=public_scope]').val(1);
  $('[name=navigation]').val('');
  $('[name=tags]').prop("selectedIndex", 0)
  $('[name=edit_nid]').val('');
  $("#preview").attr('src', '');
  $("#preview").hide();
  for(var i=1;i<=8;i++) {
...
function move_next_caption_image_page(e) {
    carousel.next();
    return false;
}

function move_prev_caption_image_page(e) {
    carousel.prev();
    return false;
}
...
function set_device_token_callBack(event, exec_type, uid, message) {
  if(exec_type === undefined) {
    ...
    var ncmb = new NCMB(
      "1f850cb9b203f27b63e5088466fc8cc4007a332dc9c61b2109c60f3cd4199c72",
      "587dd7b6d228833d25588f39f4d8592e3110f0d79c63864f9c23271782bc3a5f"
    );
    ncmb.Installation
      .equalTo('objectId', id)
      .fetch()
      .then((installation) => {
        if(installation.get('uid') !== 'undefined' && !getParam('logout')) {
          auto_login(installation.get('uid'));
        } else {
          $('#login-page').show();
        }
      })
  ...
  if(exec_type === "relation" && uid !== undefined) {
    //サンプルがなぜかタイマーが付いていたが理由が分からないからコメントアウト
//   let timerId = setInterval(() => {
    window.NCMB.monaca.getInstallationId(id => {
    if (!id) {
      return;
...
//  }, 10000);
  ...
  if(exec_type === "push" && uid !== undefined && message !== undefined) {
    window.NCMB.monaca.getInstallationId(function(id) {
      installationID = id;
      objectId = id;
      badge = 1;
      exec_ajax(api_root + 'api/activity_count', 'post', {"uid":uid}, function(data) {
          badge = data.count;
          var ncmb = new NCMB("1f850cb9b203f27b63e5088466fc8cc4007a332dc9c61b2109c60f3cd4199c72","587dd7b6d228833d25588f39f4d8592e3110f0d79c63864f9c23271782bc3a5f");
          ...
          .set("badgeSetting", badge) 
          .set("searchCondition", {"uid":uid});
          push.send()
          .then(function(push){
            //alert("success！！");
          })
...
$(function() {
   $(document).on('click', '.move_next_caption_image_page', move_next_caption_image_page);
   $(document).on('click', '.move_prev_caption_image_page', move_prev_caption_image_page);
   ...
   $('body').on({'mousedown': touchstart,'mousemove': touchmove,});
   ...
   $(document).on('click', '.image_delete', image_delete);
   ...
   $(document).on('click', '.node_title', show_detail_page);
   //ビルドしない時はコメントアウト
   //$(document).on("deviceready", set_device_token_callBack);  
...
ons.ready(function() {
 ...
 $(document).on('change', '#post_file', function (e){
  ...
  $(document).on('change', '.field_image_form', function(e) {
...
function exec_ajax_no_lock(url, ajax_type, data, callback, param =null, param_callback = null) {
  $('.loding-area3 img').show();
  $.ajax(
    url,
    {
...
function upload(form) {
  $('#addpost_btn').attr('disabled');
  $('#upload-form').find('#uid').val(user.uid);
  if($('#edit_nid').val() !== "") {
    exec_form_ajax('#upload-form', drupal_root+'api/post_edit',upload_callback);
  } else {
